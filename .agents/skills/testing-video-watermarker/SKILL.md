...
1. Generate a small input video with ffmpeg (no real footage needed):
   `mkdir -p /tmp/in && ffmpeg -f lavfi -i testsrc=duration=3:size=640x360:rate=24 /tmp/in/sample.mp4`
2. Run the tool:
...
4. Verify the watermark both visually and programmatically: pixel-diff the watermarked frame vs a frame from the input video — the diff should be large only in the watermark region (the log line "Created watermark clip with size: ... at position ..., opacity=..." gives the exact region).
...
- In some environments moviepy 2.2.1's `__version__` string incorrectly reports 2.1.2 — don't trust it for version checks (verify against the installed distribution, e.g. `pip show moviepy`).
